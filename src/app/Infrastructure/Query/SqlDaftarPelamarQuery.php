...
class SqlDaftarPelamarQuery implements DaftarPelamarQueryInterface {
    public function execute(string $lowongan_id) : array {
        $sql = "SELECT u.id as user_id, u.nama_lengkap as user_nama_lengkap, ad.diterima as lamaran_diterima, ad.dibayar as lamaran_dibayar, ad.created_at as lamaran_created_at 
            FROM lowongan l
        ...
        foreach ($result as $r) {
            $daftar_pelamar[] = new DaftarPelamarDto(
                ...
                dibayar: $r->lamaran_dibayar == 'Y' ? true : false,
